...
# Fuente: https://www.randomservices.org/random/data/Draft.html
datos_draft <- read_csv(file = "./draft_data.csv")
...
# #####################################
# 3. Scatter plot + umbral 195 +
#    anotaciones + líneas verticales
# #####################################
...
# #####################################
# 5. Barras horizontales con el nº de
#    días reclutados y sus % relativos
# #####################################
...
# #####################################
# 6. Barras verticales con la media 
#    de las papeletas por mes
# #####################################
...
# #####################################
# 7. Barras verticales de desviaciones
#   entre el número de papeleta medio y lo esperado
# #####################################
...
# #####################################
# 7. Simulación de un diagrama de
#    puntos esperado del nº medio de
#    papeleta
# #####################################


# Primero vemos como quedaría de forma uniforme
rnd_papeletas <- sample(1:366, 366, replace = FALSE)
rnd_draft <- data.frame("meses" = draft_69$meses,
                        "dias_anual" = draft_69$dias_anual,
                        "papeletas" = rnd_papeletas)
rnd_draft_stats <- rnd_draft %>% group_by(meses) %>%
  summarize(media_papeletas = mean(papeletas))

fig8 <- ggplot(rnd_draft_stats, aes(x = meses, y = media_papeletas,
                                   color = meses)) +
  geom_point(size = 8, alpha = 0.8, show.legend = FALSE) +
  scale_color_stata() + scale_fill_stata() +
  labs(title = "GRÁFICO ESPERADO", x = NULL, y = NULL) +
  theme(axis.ticks.x = element_blank(), axis.text.x = element_blank(),
        axis.ticks.y = element_blank(), axis.text.y = element_blank())

# #####################################
# 9. Regresión del mes vs número de
#    papeleta medio
# #####################################
...
# #####################################
# 11. Gráfico de cajas y bigotes + 
#     densidad + simulación Monte Carlo
# #####################################

# Fijamos semilla aleatoria para que siempre
# salga la misma generación aleatoria
set.seed(123456789)
...
# Calculamos el valor absoluto de la suma de desviaciones
# de cada simulación respecto a la media de la suma de
# desviaciones de cada simulación
